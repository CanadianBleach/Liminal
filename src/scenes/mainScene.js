...
export async function initMainScene() {
    ...
    function animate() {
        ...
        // Clamp huge frame drops (tab switch, lag spike)
        if (delta > MAX_ACCUMULATED_TIME) {
            delta = MAX_ACCUMULATED_TIME;
        }
        ...
        while (physicsAccumulator >= FIXED_TIME_STEP) {
            ...
            enemyManager.update(FIXED_TIME_STEP, player.state);
        ...
        flickeringLights.forEach(light => {
            if (Math.random() < 0.1) {
                light.intensity = 20 + Math.random() * 30;
            }
        });

        player.updateFOV(delta);
